...
// isNext: Là thẻ nằm dưới (chờ xuất hiện)
const CardItem = ({ 
    char, 
    info, 
    isTop, 
    ...
    // 1. TÍNH TOÁN VISUAL CHO THẺ TRÊN CÙNG
    const rotateDeg = dragX ? dragX * 0.05 : 0;
    
    // Logic mờ dần: Vuốt càng xa càng mờ. (300px là mốc mờ tịt)
    let opacity = 1;
    if (isTop && !exitDirection && dragX !== 0) {
        opacity = Math.max(0, 1 - Math.abs(dragX) / 300); 
    }
    // Nếu đang bay ra ngoài (bấm nút hoặc vuốt xong) thì mờ dần theo animation
    ...
    const getBorderColor = () => {
        if (!isTop) return 'white'; // Thẻ dưới luôn viền trắng
    ...
    // Style transform cho thẻ trên
    const topCardStyle = {
        transform: exitDirection 
            ? `translateX(${exitDirection === 'right' ? 200 : -200}px) rotate(${exitDirection === 'right' ? 20 : -20}deg)` // Bay đi khi xong
            : `translateX(${dragX}px) rotate(${rotateDeg}deg)`, // Di chuyển theo ngón tay
        opacity: opacity,
        zIndex: 10,
        transition: isTop && dragX !== 0 ? 'none' : 'all 0.3s ease-out', // Không delay khi đang vuốt tay
        cursor: 'grab'
    };

    // Style cho thẻ dưới (Nằm im, hơi nhỏ hơn 1 chút)
    const nextCardStyle = {
        transform: 'scale(0.95)',
        zIndex: 5,
        opacity: 1, // Thẻ dưới luôn hiện rõ visual, chỉ ẩn chữ
        filter: 'brightness(0.95)', // Hơi tối hơn chút để tạo chiều sâu
        pointerEvents: 'none' // Không cho tương tác thẻ dưới
    };
    ...
    return (
        <div 
            ...
            {...(isTop ? handlers : {})} // Chỉ gắn sự kiện vuốt cho thẻ trên
        >
            <div 
            ...
            >
                ...
                >
                    {/* QUAN TRỌNG: Thẻ dưới (isNext) sẽ ẩn chữ (opacity-0), thẻ trên hiện bình thường */}
                ...
                {/* MẶT SAU (Chỉ render nội dung nếu là thẻ trên) */}
                <div 
...
const FlashcardModal = ({ isOpen, onClose, text, dbData }) => {
    ...
    const handleNext = React.useCallback((isKnown) => {
        ...
        // Thời gian chờ animation bay đi trước khi load thẻ mới
        setTimeout(() => {
    ...
    const nextChar = queue[currentIndex + 1]; // Lấy thẻ tiếp theo để hiển thị bên dưới
    ...
    return (
        ...
        >
            <div className="w-full max-w-sm flex flex-col items-center">
                {!isFinished ? (
                    <>
                        {/* CONTAINER CHỨA THẺ - Relative để xếp chồng absolute */}
                        <div className="relative w-64 h-80 mb-6">
                            ...
                            {nextChar && (
                                <CardItem 
                                    char={nextChar}
                                    info={{}} // Không cần info vì chưa hiện
                                    isTop={false}
                                    isNext={true}
                                    isFlipped={false}
                                    dragX={0} // Thẻ dưới đứng im
                                    exitDirection={null}
                                    handlers={{}}
                                    onFlip={() => {}}
                                    btnFeedback={null}
                            ...
                            {/* Nút Back & Shuffle đặt đè lên góc dưới của container thẻ (tùy chỉnh vị trí nếu cần) */}
                        ...
                        {/* KHOẢNG TRỐNG ĐỂ BÙ CHO BUTTONS BỊ ĐẨY XUỐNG */}
                    ...
                    // MÀN HÌNH HOÀN THÀNH (GIỮ NGUYÊN)
