            ...
            {mode === 'kanji' ? (
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200 cursor-pointer" onClick={() => setIsMenuOpen(false)}>
                    ...
                    <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl border border-gray-200 overflow-hidden animate-in zoom-in-95 duration-200 cursor-default" onClick={(e) => e.stopPropagation()}>
                        ...
                        {/* --- NỘI DUNG CHÍNH (GIỮ NGUYÊN 100% LOGIC CŨ) --- */}
                        <div className="p-5 space-y-4">
                            {/* 1. MINNA NO NIHONGO */}
                            <div className="flex items-center justify-between group hover:bg-gray-50 p-2 rounded-lg transition-colors border border-dashed border-gray-200 hover:border-emerald-300">
                                <label className="text-xs font-bold text-gray-700 cursor-pointer flex items-center gap-2">
                                    <span className="text-emerald-600 text-lg">📗</span> TỪ VỰNG MINNA
                                </label>
                                <div className="flex items-center gap-2">
                                    <span className="text-gray-500 font-bold text-[10px] bg-gray-100 px-2 py-1 rounded">BÀI</span>
                                    <input type="number" min="1" max="50" placeholder="..." value={minnaLesson} onKeyDown={(e) => { if (e.key === 'Enter') { handleSmartLoadVocabulary(); return; } const allowed = ['Backspace', 'Delete', 'Tab', 'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown']; if (!/[0-9]/.test(e.key) && !allowed.includes(e.key) && !e.ctrlKey && !e.metaKey) { e.preventDefault(); } }} onChange={(e) => { setMinnaLesson(e.target.value); if (e.target.value) { setMimiN3(''); setMimiN2(''); setMimiN1(''); setTangoN3(''); setTangoN2(''); setTangoN1(''); } }} onBlur={() => { if (minnaLesson > 50) setMinnaLesson(50); if (minnaLesson < 1 && minnaLesson !== '') setMinnaLesson(1); }} className={`w-14 text-center font-bold border-b-2 focus:border-emerald-500 outline-none bg-transparent transition-all text-lg pb-0.5 ${minnaLesson !== '' ? 'text-emerald-600 border-emerald-500' : 'text-gray-400 border-gray-200'}`} />
                                </div>
                            </div>
